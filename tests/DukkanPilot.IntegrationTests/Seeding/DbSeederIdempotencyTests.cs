...
{
 [Fact] public async Task SeedAsync_is_idempotent(){await using var c=new SqliteConnection("Data Source=:memory:");await c.OpenAsync();await using(var enable=c.CreateCommand()){enable.CommandText="PRAGMA foreign_keys = ON;";await enable.ExecuteNonQueryAsync();}await using var db=new SqliteTestAppDbContext(new DbContextOptionsBuilder<AppDbContext>().UseSqlite(c).Options);await db.Database.EnsureCreatedAsync();await DbSeeder.SeedAsync(db);var a=await Counts(db);await DbSeeder.SeedAsync(db);Assert.Equal(a,await Counts(db));foreach(var slug in new[]{"demo-kafe","demo-restoran","demo-tatlici","demo-burgerci","demo-nargile"})Assert.Equal(1,await db.Businesses.CountAsync(x=>x.Slug==slug));Assert.Equal(1,await db.AppUsers.CountAsync(x=>x.Email=="admin@dukkanpilot.local"));Assert.Equal(1,await db.AppUsers.CountAsync(x=>x.Email=="owner@dukkanpilot.local"));Assert.DoesNotContain((await db.BusinessTables.Select(x=>new{x.BusinessId,x.PublicCode}).ToListAsync()).GroupBy(x=>x),x=>x.Count()>1);}
